...
contract("ERC5496Cloneable", async (accounts) => {
  ...
  it("Should set privilege 0 to Bob", async () => {
    ...
    assert.equal(user_hasP0, true, "Privilege 0 of NFT 1 should be Bob");
  });
  ...
  it("Privilege should belong to the owner by default", async () => {
    ...
    assert.equal(user_hasP1, true, "Privilege 1 of NFT 1 should be Alice");
  });
  ...
  it("The privilege holder is allowed to transfer the privilege to others", async () => {
    ...
    assert.equal(user_hasP0, true, "Privilege 0 of NFT 2 should be Bob");
    await demoContract.setPrivilege(2, 0, Tom, BigInt(expires + 100), {
      from: Bob,
    });
    user_hasP0 = await demoContract.hasPrivilege(2, 0, Tom);
    assert.equal(user_hasP0, true, "Privilege 0 of NFT 2 should be Tom");
  ...
  it("User is allowed to transfer NFT while privileges on renting", async () => {
    ...
    let owner_1 = await demoContract.ownerOf(1);
    assert.equal(owner_1, Bob, "Owner of NFT 1 should be Bob");
  ...
  it("ERC5496 cloneable", async () => {
    ...
    assert.equal(cloneable_P2, false, "privilege 2 should not be cloneable");
    let cloneable_P3 = await demoContract.cloneable(3);
    assert.equal(cloneable_P3, true, "privilege 3 should be cloneable");
    let expires = Math.floor(new Date().getTime() / 1000) + 5000;
    await demoContract.setPrivilege(4, 3, Bob, BigInt(expires), {
      from: Alice,
    });
    ...
    assert.equal(user_hasP3, true, "privilege 3 available after Bob cloned");
  });
  ...
  it("NFT should support interface IERC5496", async () => {
    ...
    for (let interfaceName in interfaceIds) {
      let isSupport = await demoContract.supportsInterface(
        interfaceIds[interfaceName]
      );
      assert.equal(
        isSupport,
        true,
        "NFT should support interface " + interfaceName
      );
    }
  });
});
